...
class Tank_auth
{
	...
	function login($login, $password, $remember, $login_by_username, $login_by_email)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!$this->ci->users->is_account_deactivated($login)) {
				
				$this->error = array('deactivated' => 'auth_message_deactivated');
	...
	function get_active_user(){
		...
		$display_name = '';
		echo json_encode(array('username' => $username, 'user_id' => $user_id, 'displayName' => $display_name));
	...
	/**
	 * Check if user diactivated.
	 *
	 * @return	bool
	 */
	function is_deactivated()
	{
		return $this->ci->session->userdata('deactivated');
	}
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $email_activation, $fb_login = false, $fb_data = array())
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			...
			if (!is_null($res = $this->ci->users->create_user($data, !$email_activation, $fb_data))) {
	...
	function reactivate($email){
		if (strlen($email) > 0) {
			if (!is_null($user = $this->ci->users->get_user_by_email($email))) {
				...
				$this->ci->users->set_deactivated($user->id);
				return $data;
	...
	private function clear_login_attempts($login)
	{
        ...
        function user_is_allowed_to_post(){
            $userid = $this->get_user_id();
            if($userid == $this->ci->users->get4DMakers_id() || $userid == $this->ci->users->get4DSystems_id()){
                return true;
            }
            return false;
        }
